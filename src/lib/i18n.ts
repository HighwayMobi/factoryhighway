...
const translations = {
  ru: {
    ...
    cp_op2Note: "Новый тариф будет применён через 3-е суток после подтверждения и оплаты. Абонентская плата €{price} будет списана сразу.",
    ...
    cp_freezeNote: "SIM-карта будет заморожена после окончания оплаченного периода текущего тарифа — {date}. Плата за заморозку €{price}/мес. Вы сможете отменить заморозку в личном кабинете до этой даты.",
    cp_whenChangeTitle: "Когда применить новый тариф?",
    cp_whenChangeNow: "В ближайшее время ({date}) — оплата сразу",
    cp_whenChangeLater: "По окончании текущего периода ({date}) — без доплаты сейчас",
    cp_cancel: "Отмена",
  ...
  en: {
    ...
    cp_freezeNote: "Your SIM will be frozen after the current paid period ends — {date}. Freeze fee €{price}/mo. You can cancel the freeze in your account until that date.",
    cp_whenChangeTitle: "When to apply the new plan?",
    cp_whenChangeNow: "Soon ({date}) — pay now",
    cp_whenChangeLater: "At the end of the current period ({date}) — no charge now",
    cp_cancel: "Cancel",
    cp_confirm: "Confirm",
